...
class ShipCloseMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: ProxyService!
    public var loadEntitiesBlock: (() async throws -> [ProxyServiceFmwk.ShipClose])?
    private var entities: [ProxyServiceFmwk.ShipClose] = .init()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let shipClose = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Matnr, Vbeln, Posnr, Kunnr", value: "\(shipClose.matnr!), \(shipClose.vbeln!), \(shipClose.posnr!), \(shipClose.kunnr!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ShipCloseDetailViewController
...
extension ShipCloseMasterViewController: ProxyServiceEntitySetUpdaterDelegate {
