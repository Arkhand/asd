...
class MaterialMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: ProxyService!
    public var loadEntitiesBlock: (() async throws -> [ProxyServiceFmwk.Material])?
    private var entities: [ProxyServiceFmwk.Material] = .init()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let material = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SKU", value: "\(material.sku!)")
        return cell
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.sku!) < ($1.sku!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! MaterialDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = entities[(tableView.indexPathForSelectedRow?.row)!].sku ?? ""
...
extension MaterialMasterViewController: ProxyServiceEntitySetUpdaterDelegate {
