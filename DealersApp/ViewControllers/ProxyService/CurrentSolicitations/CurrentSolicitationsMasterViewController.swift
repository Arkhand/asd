...
class CurrentSolicitationsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: ProxyService!
    public var loadEntitiesBlock: (() async throws -> [ProxyServiceFmwk.CurrentSolicitations])?
    private var entities: [ProxyServiceFmwk.CurrentSolicitations] = .init()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let currentSolicitations = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Matnr, Vbeln, Posnr, Kunnr", value: "\(currentSolicitations.matnr!), \(currentSolicitations.vbeln!), \(currentSolicitations.posnr!), \(currentSolicitations.kunnr!)")
        return cell
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.matnr!, $0.vbeln!, $0.posnr!, $0.kunnr!) < ($1.matnr!, $1.vbeln!, $1.posnr!, $1.kunnr!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! CurrentSolicitationsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = entities[(tableView.indexPathForSelectedRow?.row)!].matnr ?? ""
...
extension CurrentSolicitationsMasterViewController: ProxyServiceEntitySetUpdaterDelegate {
